...
class MaterialPreviewActivity : AppCompatActivity() {
    ...
    fun setPhotoPreview(path: String) {
        Picasso.with(this).load(path).into(fullscreen_content)

        fullscreen_content.visibility = View.VISIBLE

        container.setBackgroundResource(R.color.black_overlay)

    }

    fun setPhotoPreviewLocal(path: String) {
        fullscreen_content.setImageURI(Uri.parse(path))
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //formattedText.setOnClickListener { toggle() }
       ...
       // dummy_button.setOnTouchListener(mDelayHideTouchListener)
        ...
        if (intent != null){
           ...
           if(url != null)setPhotoPreview(url)
            url = intent.getStringExtra("local")
            if(url != null)setPhotoPreviewLocal(url)
        }
